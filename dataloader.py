...
class DataManager:
	...
	def get_contrastive_learning_data(self) -> [DataLoader, DataLoader, DataLoader]:
		...
		if self.data_set == 'cifar10':
			...
			# we will use the same transform across all methods for fair comparison of proposed losses
			# augmentations proposed in SimCLR-v2 : https://github.com/google-research/simclr#pretraining
			self.mv_transform = SimCLRTransform(
				input_size=self.model_ip_shape,
				cj_strength=0.5,
				gaussian_blur=0.0
			)
		else:
		...
		self.basic_transform = BasicTransform(
			mean=self.mean,
			std=self.std,
			input_shape=self.model_ip_shape
		)
